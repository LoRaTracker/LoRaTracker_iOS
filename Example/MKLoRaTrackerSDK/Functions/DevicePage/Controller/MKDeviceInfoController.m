...
- (void)loadDatasFromDevice {
    if (ValidStr(self.infoModel.batteryPower)) {
        MKContactTrackerTextCellModel *soc = self.dataList[0];
        soc.rightMsg = [self.infoModel.batteryPower stringByAppendingString:@"%"];
    }
    if (ValidStr(self.infoModel.macAddress)) {
        MKContactTrackerTextCellModel *macModel = self.dataList[1];
        macModel.rightMsg = self.infoModel.macAddress;
    }
    ...
    if (ValidStr(self.infoModel.manu)) {
        MKContactTrackerTextCellModel *manuModel = self.dataList[6];
...
- (void)loadTableDatas {
    ...
    socModel.leftMsg = @"Battery Level";
    socModel.rightMsg = @"0%";
