...
static NSString *const noteMsg = @"*The device will store the Non-Alarm BLE ADV data every 10s and the same MAC addree only be saved one time.";
...
- (void)intervalSliderValueChanged {
    self.intervalLabel.text = [NSString stringWithFormat:@"%.f s",self.intervalSlider.value];
    [self updateNoteMsg];
...
- (void)updateNoteMsg {
    self.noteLabel.text = [NSString stringWithFormat:@"*The device will store the Non-Alarm BLE ADV data every %@ and the same MAC addree only be saved one time.",self.intervalLabel.text];
}
...
- (UILabel *)msgLabel {
    if (!_msgLabel) {
        ...
        _msgLabel.attributedText = [MKAttributedString getAttributedString:@[@"Valid BLE Data Filter Interval",@"   (1-600s)"] fonts:@[MKFont(15.f),MKFont(12.f)] colors:@[DEFAULT_TEXT_COLOR,RGBCOLOR(223, 223, 223)]];
    }
    return _msgLabel;
}
...
- (MKSlider *)intervalSlider {
    if (!_intervalSlider) {
        ...
        _intervalSlider.maximumValue = 600.f;
        _intervalSlider.minimumValue = 1.f;
        _intervalSlider.value = 1.f;
