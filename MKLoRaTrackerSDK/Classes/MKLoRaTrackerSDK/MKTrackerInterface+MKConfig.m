...
+ (BOOL)isConfirmRawFilterProtocol:(id <MKRawFilterProtocol>)protocol {
    ...
    if (!MKValidStr(protocol.dataType) || protocol.dataType.length != 2 || ![self regularHexExpressions:protocol.dataType]) {
        return NO;
    }
    ...
    if (protocol.minIndex == 0 && protocol.maxIndex == 0) {
        if (!MKValidStr(protocol.rawData) || protocol.rawData.length > 58 || ![self regularHexExpressions:protocol.rawData] || (protocol.rawData.length % 2 != 0)) {
            return NO;
        }
        return YES;
    }
    ...
    if (!MKValidStr(protocol.rawData) || protocol.rawData.length > 58 || ![self regularHexExpressions:protocol.rawData]) {
        return NO;
    }
...
+ (BOOL)regularHexExpressions:(NSString *)hex{
    ...
    return [pred evaluateWithObject:hex];
}
